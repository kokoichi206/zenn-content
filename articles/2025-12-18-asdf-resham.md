...
:::message
この記事は、ジャパゲートシステムズ Advent Calendar 2025 18 日目の記事です。
:::
...
asdf で複数の Node.js バージョンを管理していたのですが、**現在 global に設定しているバージョンとは別のバージョンに `npm i -g` でインストールしたパッケージが残ってい**たのが原因でした。
...
本来、PATH に登録されるコマンドは「今使えるもの」だけであるべきで、作成時も実行時も同じバージョンを参照するのが理想的に思えます。
...
今後は **『asdf 管理下での `npm i -g` などのグローバルインストールは避けて native でのインストールに寄せていこう』**と思います。
